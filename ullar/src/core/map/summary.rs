...
impl FinalContigSummary {
    ...
    pub fn new(reference_name: String, total_samples: usize) -> Self {
        Self {
            reference_name,
            total_samples,
    ...
    pub fn summarize_matches(&mut self, data: &MappedMatrix) -> bool {
        ...
        let mut sequence_lengths = self.count_total_sequence_length(matrix);
        let total_sequence_length = sequence_lengths.iter().sum::<f64>();
        self.mean_sequence_length = self.calculate_mean_sequence_length(total_sequence_length);
        self.median_sequence_length = self.calculate_median_sequence_length(&mut sequence_lengths);
        ...
        self.percent_coverage = self.calculate_percent_coverage();

        true
    }

    fn calculate_percent_coverage(&self) -> f64 {
        (self.total_sample_matches as f64 / self.total_samples as f64) * 100.0
    }

    fn count_total_sequence_length(&self, matrix: &SeqMatrix) -> Vec<f64> {
        matrix.iter().map(|(_, seq)| seq.len() as f64).collect()
    }

    fn calculate_mean_sequence_length(&self, total_sequence_length: f64) -> f64 {
        total_sequence_length / self.total_sample_matches as f64
    }

    fn calculate_median_sequence_length(&self, sequence_lengths: &mut [f64]) -> f64 {
